...
-- groupId mínimo para GOD em TFS 1.3/1.4
local GOD_GROUP_ID = 6

function onSay(player, words, param)
    -- Permissão: apenas GOD (groupId ≥ 6)
    if player:getGroup():getId() < GOD_GROUP_ID then
        player:sendCancelMessage("Você precisa de acesso GOD para usar este comando.")
        return false
    end

    -- Parâmetro obrigatório
    local typeStr = param and trim(param) or ""
    if typeStr == "" then
        player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE,
            "Uso correto: " .. words .. " <" .. table.concat(table.keys(reloadTypes), "|") .. ">")
        return false
    end

    local key = typeStr:lower()
    local reloadType = reloadTypes[key]
    if not reloadType then
        player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE,
            "Tipo de reload '" .. key .. "' não encontrado.")
        return false
    end

    -- Se for recarregar scripts completos, limpar callbacks para evitar duplicação
