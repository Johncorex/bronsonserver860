function onSay(player, words, param)
	...
	if ret_t then
		text = text .. '\n\n         [-----Task-----]               \n      \n[*] Task atual [*]: '..ret_t.name..' - Voce precisa matar: '..ret_t.amount..'.\n[*] Premios [*]: '..getItemsFromTable(ret_t.items)..' - '..ret_t.pointsTask[1]..' Task Points - '..ret_t.pointsTask[2]..' Task Ranks. \n[*] Progresso [*]: ['..(player:getStorageValue(ret_t.storage))..'/'..ret_t.amount..']\n[*] Task Status [*]: '..(player:getStorageValue(ret_t.storage) == ret_t.amount and 'Complete' or 'Incomplete')..'!'			
	else
