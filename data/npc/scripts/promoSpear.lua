...
function creatureSayCallback(cid, type, msg)
	...
	if (msgcontains(msg, 'promotion') or msgcontains(msg, 'promote') or msgcontains(msg, 'promover'))  then
			...
			if getPlayerVocation(cid) == tabela[x] then
				if getPlayerLevel(cid) >= 35 then
					...
					elseif (getPlayerVocation(cid) == 7) and getPlayerLevel(cid) >= 101 then
						npcHandler:say('Voce quer ser promovido para Medieval Spearman por 30.000gp?', cid)
						talkState[talkUser] = 1
						
					elseif (getPlayerVocation(cid) == 12) and getPlayerLevel(cid) >= 200 then
						npcHandler:say('Voce quer ser promovido para Spartan Spearman por 200.000gp?', cid)
						talkState[talkUser] = 1
		...
		if c == 0 then
			npcHandler:say("Voce tem que ser um Royal Archer para ser promovido aqui.", cid)
		end
	...
	elseif(msgcontains(msg, 'yes') and talkState[talkUser] == 1) then
		...
		elseif(getPlayerVocation(cid) == 12) then
			if isPremium(cid) then
				...
				npcHandler:say("Voce tem que ter uma conta VIP para se tornar um Spartan Spearman!", cid)
			end
			talkState[talkUser] = 0
