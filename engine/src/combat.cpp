...
// === helpers de probabilidade (% uniformes) ===
static inline int32_t normalize_percent(int32_t raw) {
	// alguns forks guardam em basis points (0..10000 => 0.00..100.00%)
	if (raw > 100 && raw <= 10000) return raw / 100;
	return raw;
}
static inline int32_t clamp_percent(int32_t v) {
	if (v < 0) return 0;
	if (v > 100) return 100;
	return v;
}
static inline bool roll_percent(int32_t p) {
	if (p <= 0)  return false;
	if (p >= 100) return true;
	return uniform_random(1, 100) <= p;  // **uniforme** (não usar normal_random)
}
	...
	if (damage.primary.type != COMBAT_MANADRAIN) {
		...
		if (casterPlayer) {
			...
			if (!damage.critical && damage.primary.type != COMBAT_HEALING && damage.origin != ORIGIN_CONDITION) {
				uint16_t chanceRaw = casterPlayer->getSpecialSkill(SPECIALSKILL_CRITICALHITCHANCE);
				uint16_t amountRaw = casterPlayer->getSpecialSkill(SPECIALSKILL_CRITICALHITAMOUNT);

				// fallback: se só tiver chance e amount não estiver configurado, usa default
				if (amountRaw == 0 && chanceRaw > 0) {
					amountRaw = static_cast<uint16_t>(std::max<int32_t>(0, g_config.getNumber(ConfigManager::CRITICAL_AMOUNT_DEFAULT)));
				}

				int32_t critChance = clamp_percent(normalize_percent(static_cast<int32_t>(chanceRaw)));
				int32_t critAmount = std::max<int32_t>(0, static_cast<int32_t>(amountRaw));

				if (critAmount > 0 && roll_percent(critChance)) {
					const double mul = 1.0 + (critAmount / 100.0);
					damage.primary.value = std::lround(damage.primary.value * mul);
					damage.secondary.value = std::lround(damage.secondary.value * mul);
	...
	if (success) {
		...
		if (damage.critical) {
			g_game.addMagicEffect(target->getPosition(),
				static_cast<uint8_t>(g_config.getNumber(ConfigManager::CRITICAL_EFFECT)));
			g_game.addAnimatedText(g_config.getString(ConfigManager::CRITICAL_TEXT),
				target->getPosition(),
				static_cast<TextColor_t>(g_config.getNumber(ConfigManager::CRITICAL_COLOR)));
		}

		// Life / Mana Leech feedback
		if (!damage.leeched && damage.primary.type != COMBAT_HEALING && casterPlayer && damage.origin != ORIGIN_CONDITION) {
			...
			if (casterPlayer->getHealth() < casterPlayer->getMaxHealth()) {
				uint16_t lifeChanceRaw = casterPlayer->getSpecialSkill(SPECIALSKILL_LIFELEECHCHANCE);
				uint16_t lifeAmountRaw = casterPlayer->getSpecialSkill(SPECIALSKILL_LIFELEECHAMOUNT);

				if (lifeAmountRaw == 0 && lifeChanceRaw > 0) {
					lifeAmountRaw = static_cast<uint16_t>(std::max<int32_t>(0, g_config.getNumber(ConfigManager::LIFELEECH_AMOUNT_DEFAULT)));
				}

				int32_t lifeChance = clamp_percent(normalize_percent(static_cast<int32_t>(lifeChanceRaw)));
				int32_t lifeAmount = std::max<int32_t>(0, static_cast<int32_t>(lifeAmountRaw));

				if (lifeAmount > 0 && roll_percent(lifeChance)) {
					leechCombat.primary.value = std::lround(totalDamage * (lifeAmount / 100.0));
					g_game.combatChangeHealth(nullptr, casterPlayer, leechCombat);

					casterPlayer->sendMagicEffect(casterPlayer->getPosition(),
						static_cast<uint8_t>(g_config.getNumber(ConfigManager::LIFELEECH_EFFECT)));
					g_game.addAnimatedText(g_config.getString(ConfigManager::LIFELEECH_TEXT),
						casterPlayer->getPosition(),
						static_cast<TextColor_t>(g_config.getNumber(ConfigManager::LIFELEECH_COLOR)));
				}
			}

			// Mana Leech
			if (casterPlayer->getMana() < casterPlayer->getMaxMana()) {
				uint16_t manaChanceRaw = casterPlayer->getSpecialSkill(SPECIALSKILL_MANALEECHCHANCE);
				uint16_t manaAmountRaw = casterPlayer->getSpecialSkill(SPECIALSKILL_MANALEECHAMOUNT);

				if (manaAmountRaw == 0 && manaChanceRaw > 0) {
					manaAmountRaw = static_cast<uint16_t>(std::max<int32_t>(0, g_config.getNumber(ConfigManager::MANALEECH_AMOUNT_DEFAULT)));
				}

				int32_t manaChance = clamp_percent(normalize_percent(static_cast<int32_t>(manaChanceRaw)));
				int32_t manaAmount = std::max<int32_t>(0, static_cast<int32_t>(manaAmountRaw));

				if (manaAmount > 0 && roll_percent(manaChance)) {
					leechCombat.primary.value = std::lround(totalDamage * (manaAmount / 100.0));
					g_game.combatChangeMana(nullptr, casterPlayer, leechCombat);

					casterPlayer->sendMagicEffect(casterPlayer->getPosition(),
						static_cast<uint8_t>(g_config.getNumber(ConfigManager::MANALEECH_EFFECT)));
					g_game.addAnimatedText(g_config.getString(ConfigManager::MANALEECH_TEXT),
						casterPlayer->getPosition(),
						static_cast<TextColor_t>(g_config.getNumber(ConfigManager::MANALEECH_COLOR)));
				}
			}
		}
...
void Combat::doAreaCombat(Creature* caster, const Position& position, const AreaCombat* area, CombatDamage& damage, const CombatParams& params)
{
	...
	if (!damage.critical && damage.primary.type != COMBAT_HEALING && casterPlayer && damage.origin != ORIGIN_CONDITION) {
		...
		if (critAmount > 0 && roll_percent(critChance)) {
			criticalPrimary = std::lround(damage.primary.value * (critAmount / 100.0));
			criticalSecondary = std::lround(damage.secondary.value * (critAmount / 100.0));
			damage.critical = true;
		}
	}
	...
	for (Creature* creature : toDamageCreatures) {
		...
		if (success) {
			...
			if (casterPlayer && !damage.leeched && damage.primary.type != COMBAT_HEALING && damage.origin != ORIGIN_CONDITION) {
				int32_t targetsCount = static_cast<int32_t>(toDamageCreatures.size());
				...
				if (casterPlayer->getHealth() < casterPlayer->getMaxHealth()) {
					...
					if (lifeAmount > 0 && roll_percent(lifeChance)) {
						leechCombat.primary.value = static_cast<int32_t>(std::ceil(
							totalDamage * ((lifeAmount / 100.0) + ((targetsCount - 1) * ((lifeAmount / 100.0) / 10.0))) / targetsCount
						));
						g_game.combatChangeHealth(nullptr, casterPlayer, leechCombat);
				...
				if (casterPlayer->getMana() < casterPlayer->getMaxMana()) {
					...
					if (manaAmount > 0 && roll_percent(manaChance)) {
						leechCombat.primary.value = static_cast<int32_t>(std::ceil(
							totalDamage * ((manaAmount / 100.0) + ((targetsCount - 1) * ((manaAmount / 100.0) / 10.0))) / targetsCount
						));
						g_game.combatChangeMana(nullptr, casterPlayer, leechCombat);
