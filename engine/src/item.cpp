	...
	const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	// ==== helpers =========================================================
	auto printProtectionList = [&](const char* label,
		const std::vector<std::pair<std::string, int16_t>>& entries) {
			if (entries.empty()) return;
			s << "\n[" << label << ": ";
			for (size_t i = 0; i < entries.size(); ++i) {
				if (i) s << ", ";
				s << '(' << entries[i].first << ':'
					<< std::showpos << entries[i].second << std::noshowpos << "%)";
			}
			s << ']';
		};

	auto printProtectionAll = [&](const char* label, int16_t pct) {
		if (pct == 0) return;
		s << "\n[" << label << ": " << std::showpos << pct << std::noshowpos << "%]";
		};

	auto printRegen = [&](uint16_t gain, uint32_t ticks, const char* what) {
		if (gain == 0 || ticks == 0) return;
		uint32_t secs = ticks / 1000u; if (secs == 0) secs = 1u;
		s << "\n[" << what << " Regen: +" << gain << " / " << secs << "s]";
		};

	auto computeAll = [&](const int16_t* arr) -> int16_t {
		int16_t a = arr[0];
		if (a == 0) return 0;
	...
	auto collectEntries = [&](const int16_t* arr) {
		std::vector<std::pair<std::string, int16_t>> v;
		v.reserve(COMBAT_COUNT);
		for (size_t i = 0; i < COMBAT_COUNT; ++i) {
			int16_t val = arr[i];
			if (val != 0) v.emplace_back(getCombatName(indexToCombatType(i)), val);
		}
		return v;
		};

	auto emitProtections = [&](const Abilities& ab) {
		const int16_t protAll = computeAll(ab.absorbPercent.data());
		const int16_t fieldAll = computeAll(ab.fieldAbsorbPercent.data());
		const auto protList = collectEntries(ab.absorbPercent.data());
		const auto fieldList = collectEntries(ab.fieldAbsorbPercent.data());

		// DEDUPE: se ambos forem "All" com MESMO valor, imprime só uma linha
		if (protAll != 0 && fieldAll != 0 && protAll == fieldAll) {
			printProtectionAll("Protection All", protAll);
			return;
		}

		if (protAll != 0)  printProtectionAll("Protection All", protAll);
		else               printProtectionList("Protection", protList);

		if (fieldAll != 0) printProtectionAll("Protection all fields", fieldAll);
		else               printProtectionList("Protection fields", fieldList);
		};
	// ======================================================================
	...
	if (it.isRune()) {
	...
	else if (it.weaponType != WEAPON_NONE) { // ===== ARMAS =====
		// Cabeçalho apenas com Atk/Def
		int32_t attack = item ? item->getAttack() : it.attack;
		int32_t defense = item ? item->getDefense() : it.defense;
		int32_t extraDefense = item ? item->getExtraDefense() : it.extraDefense;

		if (attack != 0 || defense != 0 || extraDefense != 0) {
		...
		if (it.abilities && it.abilities->elementType != COMBAT_NONE && it.abilities->elementDamage != 0) {
			s << "\n[Element Damage: " << it.abilities->elementDamage << ' '
				<< getCombatName(it.abilities->elementType) << "]";
		}

		if (uint32_t attackSpeed = (item ? item->getAttackSpeed() : it.attackSpeed)) {
			s << "\n[Atk Spd: " << (attackSpeed / 1000.) << "s]";
		}

		if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
			uint16_t range = static_cast<uint16_t>(item ? item->getShootRange() : it.shootRange);
			if (range) {
				s << "\n[Range: " << range << "]";
			}
			int8_t hitChance = item ? item->getHitChance() : it.hitChance;
			if (hitChance != 0) {
				s << "\n[Hit Chance: " << std::showpos << static_cast<int16_t>(hitChance) << std::noshowpos << "%]";
			}
		}

		if (it.abilities) {
			for (uint8_t i = SKILL_FIRST; i <= SKILL_LAST; i++) {
				if (!it.abilities->skills[i]) continue;
				std::string label;
				switch (i) {
				case SKILL_SWORD:    label = "Sword Skill";    break;
				case SKILL_AXE:      label = "Axe Skill";      break;
				case SKILL_CLUB:     label = "Club Skill";     break;
				case SKILL_DISTANCE: label = "Distance Skill"; break;
				case SKILL_SHIELD:   label = "Shield Skill";   break;
				case SKILL_FIST:     label = "Melee Skills";   break;
				default:             label = getSkillName(i);  break;
				}
				s << "\n[" << label << ": " << std::showpos << it.abilities->skills[i] << std::noshowpos << "]";
			}


			// Special skills (native chances): print any present
			for (uint8_t i = SPECIALSKILL_FIRST; i <= SPECIALSKILL_LAST; i++) {
				if (!it.abilities->specialSkills[i]) continue;
				s << "\n[" << getSpecialSkillName(i) << ": " << std::showpos
					<< it.abilities->specialSkills[i] << std::noshowpos << "%]";
			}
			if (it.abilities->stats[STAT_MAGICPOINTS]) {
				s << "\n[Magic Level: +" << it.abilities->stats[STAT_MAGICPOINTS] << "]";
			}
			const auto& ab = *it.abilities;

			printRegen(ab.healthGain, ab.healthTicks, "Health");
			printRegen(ab.manaGain, ab.manaTicks, "Mana");

			if (it.abilities->speed) {
				s << "\n[Speed: " << std::showpos << (it.abilities->speed >> 1) << std::noshowpos << "]";
			}

			emitProtections(*it.abilities);
		}
	}
	else if (it.armor != 0 || (item && item->getArmor() != 0) || it.showAttributes) { // ===== ARMADURAS =====
		bool begin = true;
		...
		if (it.abilities) {
			...
			if (ab.stats[STAT_MAXHITPOINTS]) {
				s << "\n[Max Health: " << std::showpos << ab.stats[STAT_MAXHITPOINTS] << std::noshowpos << "]";
			}
			if (ab.statsPercent[STAT_MAXHITPOINTS]) {
				s << "\n[Max Health: " << std::showpos << ab.statsPercent[STAT_MAXHITPOINTS] << std::noshowpos << "%]";
			}
