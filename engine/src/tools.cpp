...
{
	// Máximo de "255.255.255.255" = 15 chars + '\0'
	char buffer[16];
	int res = std::snprintf(buffer, sizeof(buffer),
		"%u.%u.%u.%u",
		ip & 0xFF,
		(ip >> 8) & 0xFF,
		(ip >> 16) & 0xFF,
	...
	// res negativo => erro; res >= sizeof => truncamento
...
{
	std::tm tmsStorage;
	// usar localtime_r onde disponível para thread‑safety:
#if defined(_POSIX_THREAD_SAFE_FUNCTIONS)
	if (!localtime_r(&time, &tmsStorage)) return {};
	const std::tm* tms = &tmsStorage;
#else
	const std::tm* tms = std::localtime(&time);
	if (!tms) return {};
#endif

	// Formato fixo: "DD/MM/YYYY HH:MM:SS" = 19 chars + '\0' => buffer 20
	char buffer[20];
	int res = std::snprintf(buffer, sizeof(buffer),
