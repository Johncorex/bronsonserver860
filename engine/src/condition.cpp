	...
	if (internalManaTicks >= manaTicks) {
		...
		if (Player* player = creature->getPlayer()) {
			...
			if (isBuff && realManaGain > 0) {
				...
				g_game.addAnimatedText(std::to_string(realManaGain), player->getPosition(), TEXTCOLOR_MAYABLUE);

				SpectatorVec spectators;
				g_game.map.getSpectators(spectators, player->getPosition(), false, true);

				// Envia para os outros jogadores que estão vendo (exceto o próprio player)
				if (!spectators.empty()) {
					message.type = MESSAGE_HEALED_OTHERS;
					message.text = player->getName() + " gained " + manaGainString + " mana.";

					for (Creature* c : spectators) {
						if (Player* other = c->getPlayer()) {
							if (other == player) {
								continue; // pula o próprio
							}
							other->sendTextMessage(message);
						}
					}
				}
			}
