...
void Player::onChangeZone(ZoneType_t zone)
{
	if (zone == ZONE_PROTECTION) {
		...
		// ao entrar em PZ, limpa combate (target e condição)
		 if (!hasFlag(PlayerFlag_IgnoreProtectionZone)) {
			            // remove alvo de combate
				if (attackedCreature) {
				setAttackedCreature(nullptr);
				onAttackedCreatureDisappear(false);
				
			}
			            // remove status de inFight, reiniciando logout
...
uint16_t Player::getSpecialSkill(uint8_t skill) const {
	int32_t total = varSpecialSkills[skill]; // Conditions/Lua

	static const slots_t SLOTS[] = {
	...
	if (g_config.getBoolean(ConfigManager::APPLY_ITEM_SPECIALSKILLS)) {
		for (const auto s : SLOTS) {
			const Item* item = inventory[s];
			if (!item) continue;
			...
			if (it.abilities) {
				// Usa APENAS abilities...
				add += it.abilities->specialSkills[skill];
			}
			else {
				// ...OU o fallback dinâmico (nunca os dois)
	#ifdef ITEM_ATTRIBUTE_SPECIALSKILL_CRITICALHITCHANCE
				if (skill == SPECIALSKILL_CRITICALHITCHANCE) {
					add += item->getIntAttr(ITEM_ATTRIBUTE_SPECIALSKILL_CRITICALHITCHANCE);
				}
	#endif
	#ifdef ITEM_ATTRIBUTE_SPECIALSKILL_LIFELEECHCHANCE
				if (skill == SPECIALSKILL_LIFELEECHCHANCE) {
					add += item->getIntAttr(ITEM_ATTRIBUTE_SPECIALSKILL_LIFELEECHCHANCE);
				}
	#endif
	#ifdef ITEM_ATTRIBUTE_SPECIALSKILL_MANALEECHCHANCE
				if (skill == SPECIALSKILL_MANALEECHCHANCE) {
					add += item->getIntAttr(ITEM_ATTRIBUTE_SPECIALSKILL_MANALEECHCHANCE);
				}
	#endif
			}
	...
	if (total < 0) total = 0;
	if (skill == SPECIALSKILL_CRITICALHITCHANCE ||
		skill == SPECIALSKILL_LIFELEECHCHANCE ||
		skill == SPECIALSKILL_MANALEECHCHANCE) {
