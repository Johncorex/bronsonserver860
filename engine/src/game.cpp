...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		if (realHealthChange > 0 && !target->isInGhostMode()) {
			...
			addAnimatedText(strHealthChange.str(), targetPos, TEXTCOLOR_PASTELRED);
		...
		if (targetPlayer && target->hasCondition(CONDITION_MANASHIELD) && damage.primary.type != COMBAT_UNDEFINEDDAMAGE) {
			...
			if (manaDamage != 0) {
				...
				// >>> palavras conforme crítico <<<
				const char* yourAttackWord = damage.critical ? "your critical attack" : "your attack";
				const char* yourOwnAttackWord = damage.critical ? "your own critical attack" : "your own attack";
				const char* anAttackWord = damage.critical ? "a critical attack" : "an attack";
				// <<<
				...
				for (Creature* spectator : spectators) {
					...
					if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
						message.type = MESSAGE_STATUS_DEFAULT;
						message.text = fmt::format("{:s} loses {:d} mana due to {:s}.",  // [CHANGED]
							target->getNameDescription(), manaDamage, yourAttackWord);
						...
						if (!attacker) {
							...
							message.text = fmt::format("You lose {:d} mana due to {:s}.", // [CHANGED]
								manaDamage, yourOwnAttackWord);
						} else {
							message.text = fmt::format("You lose {:d} mana due to {:s} by {:s}.", // [CHANGED]
								manaDamage, anAttackWord, attacker->getNameDescription());
						}
					} else {
						continue;
					}
		...
		if (message.primary.color != TEXTCOLOR_NONE || message.secondary.color != TEXTCOLOR_NONE) {
			...
			// [ADDED] palavras que variam conforme crítico
			const char* yourAttackWord = damage.critical ? "your critical attack" : "your attack";
			const char* yourOwnAttackWord = damage.critical ? "your own critical attack" : "your own attack";
			const char* anAttackWord = damage.critical ? "a critical attack" : "an attack";
			...
			for (Creature* spectator : spectators) {
				...
				if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
					message.type = MESSAGE_STATUS_DEFAULT;
					message.text = fmt::format("{:s} loses {:s} due to {:s}.",             // [CHANGED]
						target->getNameDescription(), damageString, yourAttackWord);
					message.text[0] = std::toupper(message.text[0]);
				} else if (tmpPlayer == targetPlayer) {
					message.type = MESSAGE_STATUS_DEFAULT;
					if (!attacker) {
						...
						message.text = fmt::format("You lose {:s} due to {:s}.",            // [CHANGED]
							damageString, yourOwnAttackWord);
					} else {
						message.text = fmt::format("You lose {:s} due to {:s} by {:s}.",    // [CHANGED]
							damageString, anAttackWord, attacker->getNameDescription());
					}
				} else {
					continue;
				}
	...
	if (manaChange > 0) {
		...
		for (Creature* spectator : spectators) {
			...
			if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
				message.type = MESSAGE_STATUS_DEFAULT;
				message.text = fmt::format("{:s} loses {:d} mana due to {:s}.",         // [CHANGED]
					target->getNameDescription(), manaLoss, yourAttackWord);
				message.text[0] = std::toupper(message.text[0]);
			} else if (tmpPlayer == targetPlayer) {
				message.type = MESSAGE_STATUS_DEFAULT;
				if (!attacker) {
					...
					message.text = fmt::format("You lose {:d} mana due to {:s}.",        // [CHANGED]
						manaLoss, yourOwnAttackWord);
				} else {
					message.text = fmt::format("You lose {:d} mana due to {:s} by {:s}.",// [CHANGED]
						manaLoss, anAttackWord, attacker->getNameDescription());
				}
			} else {
				continue;
			}
